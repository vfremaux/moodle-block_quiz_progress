...
class block_quiz_progress extends block_base {
    ...
    function get_content() {
		...
		// check JQPLot is available
        if(!is_dir($CFG->libdir.'/jqplot')) {
            $this->content->text = get_string('errornojqplot', 'block_quiz_progress');
            return $this->content;
        }
        
		// preconfigures the quiz type to default standard quiz
        if (empty($this->config->quiztype)){
            $this->config->quiztype = get_field('modules', 'id', 'name', 'quiz');
        ...
        if(empty($this->config->quizid)){
        	...
        	$qinstances = count_records($quiztypename, 'course', $COURSE->id);
        	if ($qinstances == 0){
        		$this->content->text .= '<span class="error">'.get_string('errornoquiz', 'block_quiz_progress'). '</span>';
            	return $this->content;
        	} elseif ($qinstances == 1){
        		$this->config->quizid = get_field($quiztypename, 'id', 'course', $COURSE->id);
            	$this->instance_config_commit();        	
        	} else {
        ...
        $cm = get_record('course_modules', 'module', $this->config->quiztype, 'instance', $this->config->quizid);
        $modcontext = get_context_instance(CONTEXT_MODULE, $cm->id);

        // Get the quiz record
        $quiz = get_record($quiztypename, 'id', $this->config->quizid);
        if(empty($quiz)) {
        ...
        $attemptgrades = get_records_select($quiztypename.'_attempts', " {$quiztypename} = {$this->config->quizid} AND timefinish != 0 AND userid = {$USER->id} ", 'timefinish ASC', 'id, sumgrades, timefinish');
        ...
        if (!empty($data)){
	  	...
	  	$this->content->text .= "<link rel=\"stylesheet\" type=\"text/css\" href=\"{$CFG->wwwroot}/lib/jqplot/jquery.jqplot.css\" />";
	  	$this->content->text .= "
	...
	/**
	* we might need recode the stored linked test id against backup_ids.
	*
	*/
